...
Features Camera Motion Compensation (GMC) & ReID for Aerial Drone Video
"""
...
class BoTSORTTracker(BaseTracker):
    ...
    Optimized for aerial surveillance where camera ego-motion (yaw, tilt, drift) occurs.
    """
    ...
    def __init__(
        ...
        # Internal track state history to compute velocity, heading, and longevity
        ...
        if results and len(results) > 0:
            ...
            if boxes is not None:
                for i in range(len(boxes)):
                    box = boxes[i]
                    if box.id is None:
                        continue  # Detection not yet associated with a track
                    ...
                    if track_id in self._prev_centroids:
                        ...
                        # Heading in degrees (0 = East, 90 = South, 180 = West, 270 = North in image space)
                        if dist > 1.0:
